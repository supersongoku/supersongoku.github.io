...
function verifyResult(pOptionID, pAnswerID, pAnswerText, failed_list){
	var result = "";
	// check answer
	clearAllTimeout();
...
function LoadQuestion(failed_list){
	var noofquestions = 0;
    ...
    $(document).ready(function(){
	    $.ajax({
	      ...
	      success: function(xml){
			...
			var selectedALLQID = Math.floor(Math.random() * noofquestions) + 1;

			var selectedFailedQIDIndex = Math.floor(Math.random() * failed_list.length);
			var selectedFailedQID = failed_list[selectedFailedQIDIndex];
			
			var selectedQID;
			if Math.floor(Math.random()) > 0.5 {
				selectedQID = selectedALLQID;
			}else{
				selectedQID = selectedFailedQID;
			};
